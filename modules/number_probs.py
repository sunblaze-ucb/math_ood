...
def list_prime_factors(min_second_prime, sample_args, context=None):
  """E.g., "What is the second-largest prime factor of 36?"."""
  if context is None:
    context = composition.Context()
  ...
  # We'll use a helper function to check if a number meets our criteria
  def has_valid_second_largest_prime_factor(n):
    prime_factors = sorted(sympy.factorint(n).keys())
    # If there's only one prime factor, return False as there's no second largest
    if len(prime_factors) <= 1:
      return False
    # Check if the second largest prime factor is at least 100
    return prime_factors[-2] >= min_second_prime

  # Generate numbers until we find one with a valid second largest prime factor
  max_attempts = 1000
  for _ in range(max_attempts):
    integer = number.integer(entropy, signed=False, min_abs=2)
    if has_valid_second_largest_prime_factor(integer):
      break
  else:
    # If we couldn't find a suitable number after max_attempts, use a predefined one
    # This is a fallback with prime factors [101, 103]
    raise Exception('No prime factors found, use a larger entropy')

  (entity,) = context.sample(sample_args, [integer])
  prime_factors = sorted(sympy.factorint(integer).keys())
  second_largest_prime = prime_factors[-2]
  template = random.choice([
      'What is the second-largest prime factor of {integer}?',
      'Find the second-largest prime factor of {integer}.',
  ])
  return example.Problem(
      question=example.question(
          context, template, integer=entity.expression_else_handle),
      answer=second_largest_prime)
...
def gcd(value, sample_args, context=None):
  ...
  entropy, sample_args = sample_args.peel()
  if value is None:
    value_entropy = 1 + random.uniform(max(entropy/3-2, 0), entropy/3)
