...
def _calculate(value, sample_args, context, add_sub, mul_div, length=None):
  ...
  if is_question:
    template = random.choice([
        '{op} Express your answer as an integer or in the form p/q where p and q are coprime.',
        'What is {op}? Express your answer as an integer or in the form p/q where p and q are coprime.',
        'Evaluate {op}. Express your answer as an integer or in the form p/q where p and q are coprime.',
        'Calculate {op}. Express your answer as an integer or in the form p/q where p and q are coprime.',
        'What is the value of {op}? Express your answer as an integer or in the form p/q where p and q are coprime.',
    ])
    return example.Problem(
...
def _sample_surd(base, entropy, max_power, multiples_only, min_depth=1, max_depth=3, current_depth=0):
    ...
    *   1 / (1 + sqrt(3))  (reduces to -1/2 + (-1/2) sqrt(3))
    ...
    Args:
      ...
      multiples_only: Whether the surd should be an integer multiple of
          sqrt(base).
      min_depth: Minimum recursion depth required.
    ...
    Returns:
      Instance of `ops.Op`.
    """
    # Only return a simple sqrt if we've reached max depth AND minimum depth requirement
    if entropy <= 0 or (current_depth >= max_depth and current_depth >= min_depth):
    ...
    def power():
        ...
        surd = _sample_surd(base, entropy, max_power=1, multiples_only=False,
                            min_depth=min_depth, max_depth=max_depth, current_depth=current_depth + 1)
        return ops.Pow(surd, 2)

    # Handle depth constraints
    if current_depth >= max_depth - 1:
        # Near max depth, prioritize simpler operations
        choices = [mul_by_integer]  # Simplest operation
    elif current_depth < min_depth - 1:
        # Below min depth, force more complex operations to reach min depth
        choices = [add_or_sub_]  # Ensure we add more depth
        ...
        # Between min and max depth, use all operations
...
def simplify_surd(max_power, min_depth, max_depth, sample_args, context=None, ):
    """E.g., "Simplify (2 + 5*sqrt(3))**2."

    Args:
      max_power: Integer >= 1; the max power used in expressions.
      sample_args: Arguments for sampling.
      context: Composition context.
      min_depth: Minimum recursion depth to ensure expression complexity.
      max_depth: Maximum recursion depth to limit expression complexity.
    """
    if context is None:
        context = composition.Context()
    ...
    # Ensure min_depth doesn't exceed max_depth
